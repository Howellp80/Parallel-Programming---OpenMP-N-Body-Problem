...
int main( int argc, char *argv[ ] ){
	...
	// output labels...
	//std::cout << "Course Grained parallelism ";
	std::cout << "Fine Grained parallelism ";
	...
	for( int t = 0; t < NUMSTEPS; t++ ) {
		...
		for( int i = 0; i < NUMBODIES; i++ ) {
			...
			Bodies[i].xnew = Bodies[i].x + Bodies[i].vx*TIMESTEP +
								  0.5*ax*TIMESTEP*TIMESTEP;
			Bodies[i].ynew = Bodies[i].y + Bodies[i].vy*TIMESTEP +
								  0.5*ay*TIMESTEP*TIMESTEP;
			Bodies[i].znew = Bodies[i].z + Bodies[i].vz*TIMESTEP +
								  0.5*az*TIMESTEP*TIMESTEP;
	...
	double runTime = time1 - time0;
	
	float megaBodies = ((float)(NUMBODIES * NUMBODIES * NUMSTEPS) / 
							 (time1 - time0) / 1000000.);
	
	// print performance to console
	std::cout << "Using " << NUMT << " threads" << std::endl;
	std::cout << "MegaBodies per second: \t" << megaBodies << std::endl;
	std::cout << "Total run-time: \t" << runTime << std::endl << std::endl;
